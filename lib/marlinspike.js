...
export default class Marlinspike {
  ...
  load(config, next) {
    if (config.marlinspike.services) this.loadServices()
    if (config.marlinspike.models) this.loadModels()
    if (config.marlinspike.controllers) this.loadControllers()
    if (config.marlinspike.policies) this.loadPolicies()
    this.sails.emit(`hook:${this.name}:loaded`)
    next();
  }
  ...
  static createSailsHook (Hook) {
    return sails => {
      ...
      return {
        ...
        initialize (next) {
          sails.after(`hook:${hook.name}:configured`, () => {
